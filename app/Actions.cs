...
public class Actions
{
    ...
    //Original route /new-word
        app.MapPost("/new-player", async (HttpContext context) =>
        {
            ...
            bool success = await NewPlayer(requestBody.Word, context.Request.Cookies["ClientId"]);
            return success ? Results.Ok("Word added successfully.") : Results.StatusCode(500);
        });

        // Process incomming NewWord  from client
        async Task<bool> NewPlayer(string name, string cookie)
        {
            await using var cmd = db.CreateCommand("INSERT INTO player (name, cookie) VALUES ($1, $2)");
        ...
        // app.MapGet("/add-player/{player}", TestWord);
        //
